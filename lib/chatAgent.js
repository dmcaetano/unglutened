 ...
 */
async function executeTool(name, args, actions) {
  switch (name) {
    case 'get_today': {
      const now = new Date();
      return {
    ...
    case 'list_meals': {
      ...
      const meals = await store.listMeals(opts);
    ...
    case 'list_symptoms': {
      ...
      if (args.to != null) opts.to = args.to;
      if (args.limit != null) opts.limit = args.limit;
      const symptoms = await store.listSymptoms(opts);
    ...
    case 'update_meal': {
      ...
      const meal = await store.updateMeal(args.id, fields);
      if (!meal) {
    ...
    case 'update_symptom': {
      ...
      const symptom = await store.updateSymptom(args.id, fields);
      if (!symptom) {
    ...
    case 'delete_meal': {
      ...
      const ok = await store.deleteMeal(args.id);
      if (!ok) {
        return { result: { ok: false, error: `No meal with id ${args.id}` } };
      }
      const action = {
    ...
    case 'delete_symptom': {
      ...
      const ok = await store.deleteSymptom(args.id);
      if (!ok) {
        return { result: { ok: false, error: `No gut entry with id ${args.id}` } };
      }
      const action = {
    ...
    case 'get_correlations': {
      const meals = await store.listMeals({ limit: 1000 });
      const symptoms = await store.listSymptoms({ limit: 1000 });
...
async function runChat({ message, history = [] } = {}) {
      ...
      if (toolCalls && toolCalls.length) {
        ...
        for (const call of toolCalls) {
          ...
          if (!parsed.ok) {
            ...
            try {
              const out = await executeTool(name, parsed.args, actions);
  ...
  try {
    await store.saveChat('user', userMessage);
    await store.saveChat('assistant', reply);
  } catch (_) {
