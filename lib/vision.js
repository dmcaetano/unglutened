...
const { VISION_MODEL, CHAT_MODEL } = openrouter;

// FROZEN shared taxonomy — must match the frontend verbatim.
...
const TEXT_SYSTEM_PROMPT = [
  'You are a nutrition assistant. Given a meal name/description, infer the most likely ingredients of a typical version of that dish and flag common FOOD ALLERGENS and digestive irritants.',
  'Use real-world recipe knowledge — e.g. sushi → rice, fish (often raw → histamine), nori, SOY SAUCE (contains soy AND wheat/gluten), sometimes sesame/shellfish; pesto → basil, PINE/TREE NUTS, parmesan (dairy), garlic.',
  'Only flag what is genuinely likely.',
  'For every ingredient provide:',
  `- "name": short common name of the food/ingredient.`,
  `- "category": one of ${CATEGORIES.join(', ')}.`,
  `- "irritant": boolean — true for common food allergens and digestive irritants.`,
  `- "irritant_type": when irritant is true, one of ${IRRITANT_TYPES.join(', ')}; otherwise null. irritant_type MUST be one of the allowed list.`,
  `- "confidence": a number from 0 to 1 for how likely this ingredient is present in a typical version of the dish.`,
  'Also provide a short "title" (a concise meal name) and a one-sentence "summary" describing the meal.',
  'Return STRICT JSON only — no prose, no markdown fences — shaped exactly as:',
  '{"title": string, "summary": string, "ingredients": [{"name": string, "category": string, "irritant": boolean, "irritant_type": string|null, "confidence": number}]}',
].join('\n');
...
/**
 * Infer a meal's likely ingredients from its NAME / DESCRIPTION only (no image).
 * Uses the cheap text model (CHAT_MODEL, DeepSeek) via OpenRouter.
 * Same return shape as analyzeMeal. Never throws.
 *
 * @param {Object} opts
 * @param {string} [opts.title]       - meal name / title
 * @param {string} [opts.description] - optional free-text description
 * @returns {Promise<Object>} { title, summary, ingredients, irritant_flags, [error] }
 */
async function analyzeMealText({ title, description } = {}) {
  const fallbackTitle = (title && String(title).trim()) || 'Meal';
  ...
  try {
    ...
    if (!titleStr && !descStr) {
      return {
        ...
        ingredients: [],
        irritant_flags: [],
        error: 'No meal name or description provided',
      };
    }

    const parts = [];
    if (titleStr) parts.push(`Meal name: ${titleStr}`);
    if (descStr) parts.push(`Description: ${descStr}`);
    const userText =
      parts.join('\n') +
      '\n\nInfer the likely ingredients of a typical version of this meal and return strict JSON as specified.';
    ...
    if (!parsed || typeof parsed !== 'object') {
      return {
        ...
        ingredients: [],
        irritant_flags: [],
        error: 'Could not parse text model response',
      };
    }
...
module.exports = { analyzeMeal, analyzeMealText, IRRITANT_TYPES };
